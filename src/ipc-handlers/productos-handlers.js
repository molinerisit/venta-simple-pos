...
const { TAXONOMIA, sugerirClasificacion } = require("../data/taxonomia");

async function seedTaxonomia(ProductoDepartamento, ProductoFamilia) {
  for (const depto of TAXONOMIA) {
    const [deptoRecord] = await ProductoDepartamento.findOrCreate({
      where: { nombre: depto.nombre },
      defaults: { nombre: depto.nombre },
    });
    for (const familia of depto.familias) {
      await ProductoFamilia.findOrCreate({
        where: { nombre: familia.nombre, DepartamentoId: deptoRecord.id },
        defaults: { nombre: familia.nombre, DepartamentoId: deptoRecord.id },
      });
    }
  }
}
function registerProductosHandlers(models, sequelize) {
  ...
  seedTaxonomia(ProductoDepartamento, ProductoFamilia).catch((e) =>
    console.error("[taxonomia] Error sembrando taxonomia:", e)
  );
  // SKU automatico
  ...
  ipcMain.handle("get-productos", async (_event, opts) => {
    ...
    // Productos de acceso rÃ¡pido (para la barra de la caja)
...
// Obtener un producto por ID
  ...
  // CatÃ¡logo (departamentos + familias)
  ...
  // Suggest departamento/familia based on product name using predefined taxonomy
  ipcMain.handle("suggest-clasificacion", async (_event, nombreProducto) => {
    try {
      const sugerencia = sugerirClasificacion(nombreProducto);
      if (!sugerencia) return null;
      const [depto, familia] = await Promise.all([
        ProductoDepartamento.findOne({ where: { nombre: sugerencia.departamento }, raw: true }),
        ProductoFamilia.findOne({ where: { nombre: sugerencia.familia }, raw: true }),
      ]);
      ...
      return {
        ...
        familiaNombre: familia ? familia.nombre : null,
      };
    } catch (e) {
      console.error("[suggest-clasificacion]", e);
      return null;
    }
  });
  ...
  // ESTA ES LA FUNCIÃ“N REAL DE GUARDADO (LA QUE ARREGLAMOS)
  ipcMain.handle("guardar-producto", async (_event, productoData) => {
    const t = await sequelize.transaction();
    try {
      // M-7: explicit allowlist â€” renderer cannot inject arbitrary fields.
      ...
      // L-8: Reject empty nombre early â€” gives a clearer error than ORM notEmpty.
      if (!payload.nombre) {
      ...
      // ==========================================================
      // ðŸŸ¢ INICIO DE LA CORRECCIÃ“N
      // Esta lÃ³gica diferencia explÃ­citamente entre ACTUALIZAR (update) y CREAR (create).
      // ==========================================================
      ...
      if (error.name === "SequelizeUniqueConstraintError") {
        ...
        if (campo === "codigo") {
          return { success: false, message: "El 'CÃ³digo' ya estÃ¡ en uso. Debe ser Ãºnico." , error: true };
        }
      }
      console.error("Error al guardar producto:", error);
      return { success: false, message: error.message || "OcurriÃ³ un error inesperado al guardar." , error: true };
    }
  });
  ...
  ipcMain.handle("toggle-producto-activo", async (_event, productoId) => {
    try {
      // L-4: Single-query toggle â€” eliminates the SELECT+UPDATE round-trip.
  ...
  // ==========================================================
    ...
    // H-5b: "show-open-dialog" removed â€” renderer-controlled generic dialog is an
  ...
  ipcMain.handle("export-productos-csv", async () => {
    try {
      ...
      if (canceled || !filePath) {
        return { success: false, message: "ExportaciÃ³n cancelada." , error: true };
      }
  ...
  ipcMain.handle("import-productos-csv", async () => {
    ...
    try {
      ...
      if (canceled || !filePaths || filePaths.length === 0) {
        return { success: false, message: "ImportaciÃ³n cancelada." , error: true };
      }
      ...
      if (!productosCSV || productosCSV.length === 0) {
        return { success: false, message: "El archivo CSV estÃ¡ vacÃ­o o tiene un formato incorrecto." , error: true };
      }

      // M-11: Reject oversized imports to prevent memory exhaustion.
      if (productosCSV.length > 10000) {
        return { success: false, message: `El CSV tiene ${productosCSV.length} filas. El lÃ­mite es 10.000 por lote.`, error: true };
      }
      // H-7: Fully atomic â€” all findOrCreate (depts/families) and bulkCreate
      ...
      await sequelize.transaction(async (t) => {
        ...
        await Producto.bulkCreate(productosParaGuardar, {
          // H-6: 'stock' excluded â€” CSV import must not overwrite existing stock.
          updateOnDuplicate: [
